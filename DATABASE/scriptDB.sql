...
CREATE TABLE IF NOT EXISTS `EPS_SYSTEM`.`CORRECTION` (
  ...
  `status` TINYINT NULL,
  `type` ENUM('BIBLIOGRAPHY', 'OBJETIVES', 'SPECIFIC_OBJETIVES', 'CALENDAR', 'PLAN', 'ANEXO', 'TITLE', 'COORDINATE', 'OTHER') NOT NULL,
  `USER_userId` VARCHAR(45) NOT NULL,
  `SECTION_id` INT NULL,
  `PROJECT_id` INT NOT NULL,
  PRIMARY KEY (`id`),
...
CREATE INDEX `fk_table1_USER1_idx` ON `EPS_SYSTEM`.`CORRECTION` (`USER_userId` ASC) VISIBLE;

CREATE INDEX `fk_CORRECTION_SECTION1_idx` ON `EPS_SYSTEM`.`CORRECTION` (`SECTION_id` ASC) VISIBLE;

CREATE INDEX `fk_CORRECTION_PROJECT1_idx` ON `EPS_SYSTEM`.`CORRECTION` (`PROJECT_id` ASC) VISIBLE;
