...
CREATE TABLE IF NOT EXISTS `EPS_SYSTEM`.`PROJECT` (
  ...
  `CORRECTION_Calendario` INT NULL DEFAULT NULL,
  `CORRECTION_Plan` INT NULL DEFAULT NULL,
  `CORRECTION_Anexo` INT NULL DEFAULT NULL,
  `CORRECTION_title` INT NULL DEFAULT NULL,
  `CORRECTION_objectives` INT NULL DEFAULT NULL,
  `CORRECTION_coordinate` INT NULL DEFAULT NULL,
  `CORRECTION_specificObjectives` INT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_PROJECT_CORRECTION1`
    FOREIGN KEY (`CORRECTION_Calendario`)
    REFERENCES `EPS_SYSTEM`.`CORRECTION` (`id`),
  CONSTRAINT `fk_PROJECT_CORRECTION2`
    FOREIGN KEY (`CORRECTION_Plan`)
    REFERENCES `EPS_SYSTEM`.`CORRECTION` (`id`),
  CONSTRAINT `fk_PROJECT_CORRECTION3`
    FOREIGN KEY (`CORRECTION_Anexo`)
    REFERENCES `EPS_SYSTEM`.`CORRECTION` (`id`),
  CONSTRAINT `fk_PROJECT_CORRECTION4`
    FOREIGN KEY (`CORRECTION_title`)
    REFERENCES `EPS_SYSTEM`.`CORRECTION` (`id`),
  CONSTRAINT `fk_PROJECT_CORRECTION5`
    FOREIGN KEY (`CORRECTION_objectives`)
    REFERENCES `EPS_SYSTEM`.`CORRECTION` (`id`),
  CONSTRAINT `fk_PROJECT_CORRECTION6`
    FOREIGN KEY (`CORRECTION_coordinate`)
    REFERENCES `EPS_SYSTEM`.`CORRECTION` (`id`),
  CONSTRAINT `fk_PROJECT_PROCESS1`
    FOREIGN KEY (`PROCESS_id`)
    REFERENCES `EPS_SYSTEM`.`PROCESS` (`id`),
  CONSTRAINT `fk_PROJECT_CORRECTION7`
    FOREIGN KEY (`CORRECTION_specificObjectives`)
    REFERENCES `EPS_SYSTEM`.`CORRECTION` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
...
CREATE INDEX `fk_PROJECT_CORRECTION1_idx` ON `EPS_SYSTEM`.`PROJECT` (`CORRECTION_Calendario` ASC);

CREATE INDEX `fk_PROJECT_CORRECTION2_idx` ON `EPS_SYSTEM`.`PROJECT` (`CORRECTION_Plan` ASC);

CREATE INDEX `fk_PROJECT_CORRECTION3_idx` ON `EPS_SYSTEM`.`PROJECT` (`CORRECTION_Anexo` ASC);

CREATE INDEX `fk_PROJECT_CORRECTION4_idx` ON `EPS_SYSTEM`.`PROJECT` (`CORRECTION_title` ASC);

CREATE INDEX `fk_PROJECT_CORRECTION5_idx` ON `EPS_SYSTEM`.`PROJECT` (`CORRECTION_objectives` ASC);

CREATE INDEX `fk_PROJECT_CORRECTION6_idx` ON `EPS_SYSTEM`.`PROJECT` (`CORRECTION_coordinate` ASC);

CREATE INDEX `fk_PROJECT_CORRECTION7_idx` ON `EPS_SYSTEM`.`PROJECT` (`CORRECTION_specificObjectives` ASC);
