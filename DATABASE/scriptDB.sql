...
-- -----------------------------------------------------
-- Table `EPS_SYSTEM`.`OBSERVATION`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `EPS_SYSTEM`.`OBSERVATION` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `text` BLOB NOT NULL,
  `date` DATE NOT NULL,
  `USER` VARCHAR(45) NOT NULL,
  `REQUERIMENT` INT(11) NOT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_OBSERVATION_USER1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_OBSERVATION_REQUERIMENT1`
    FOREIGN KEY (`REQUERIMENT`)
    REFERENCES `EPS_SYSTEM`.`REQUERIMENT` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_OBSERVATION_USER1_idx` ON `EPS_SYSTEM`.`OBSERVATION` (`USER` ASC) ;

CREATE INDEX `fk_OBSERVATION_REQUERIMENT1_idx` ON `EPS_SYSTEM`.`OBSERVATION` (`REQUERIMENT` ASC) ;
...
CREATE TABLE IF NOT EXISTS `EPS_SYSTEM`.`CORRECTION` (
  ...
  `date` DATE NOT NULL,
  `status` TINYINT NOT NULL,
  `type` ENUM('TITLE', 'GENERAL_OBJETIVES', 'SPECIFIC_OBJETIVES', 'SECTION', 'COORDINATE','SCHEDULE', 'INVESTMENT_PLAN', 'BIBLIOGRAPHY' , 'ANNEXED','REJECTED') NOT NULL,
  `USER_userId` VARCHAR(45) NOT NULL,
  `SECTION_id` INT NULL,
  `PROJECT_id` INT NOT NULL,
  PRIMARY KEY (`id`),
