...
CREATE INDEX `fk_PROJECT_BIBLIOGRAPHY1_idx` ON `EPS_SYSTEM`.`PROJECT` (`BIBLIOGRAPHY_id` ASC) ;
...
CREATE INDEX `fk_REQUERIMENT_PREPROJECT1_idx` ON `EPS_SYSTEM`.`REQUERIMENT` (`PREPROJECT_id` ASC) ;
...
CREATE TABLE IF NOT EXISTS `EPS_SYSTEM`.`PROCESS` (
  ...
  CONSTRAINT `fk_PROCESS_REQUERIMENT1`
    ...
    REFERENCES `EPS_SYSTEM`.`REQUERIMENT` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE INDEX `fk_PROCESS_REQUERIMENT1_idx` ON `EPS_SYSTEM`.`PROCESS` (`REQUERIMENT_id` ASC) ;
...
CREATE INDEX `fk_PROCESS_USER_CAREER1_idx` ON `EPS_SYSTEM`.`PROCESS` (`USER_CAREER_id` ASC) ;
...
CREATE INDEX `fk_TITULO_TITULO1_idx` ON `EPS_SYSTEM`.`TITLE` (`TITLE_id` ASC) ;
...
CREATE INDEX `fk_SECTION_TITLE1_idx` ON `EPS_SYSTEM`.`SECTION` (`TITLE_id` ASC) ;
